...
- **`clear_opquast_tables.py`** : vide les tables du référentiel Opquast sur `POSTGRES_DB`. Destructif : il annonce le nombre de règles et la base visée, et n'agit qu'après un `y` explicite (Entrée, « oui » ou une entrée fermée refusent ; même convention que `ingestion.py`). **Plus de cible `make`** depuis le 2026-09-20 : il ne se lance qu'à la main. Tests : `tests/unit/scripts/`.
