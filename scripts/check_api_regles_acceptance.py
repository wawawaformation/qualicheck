...
dédié). Vérifie les volumes réels par filtre
(tests/acceptance/api_regles_acceptance.jsonl), l'authentification du PATCH,
et la boucle de revue de bout en bout (annotation réelle sur une règle,
vérifiée par scripts/enrich_again.py --dry-run, puis annotation retirée).
...
from app.api_regles.acceptance import (  # noqa: E402
    evaluate_case,
    is_acceptable,
    load_cases,
...
def main() -> None:
        ...
        with httpx.Client() as client:
            ...
            for evaluation in evaluations:
                ...
                progress_logger.info(
                    ...
                    f"(attendu {evaluation['nombre_attendu']}, "
                    f"obtenu {evaluation['nombre_retourne']}) — {statut}"
                )

            patch_refuse = verifier_patch_sans_token_refuse(client, base_url)
            progress_logger.info(
                f"check_api_regles_acceptance — PATCH sans jeton refusé (401) — "
                f"{'OK' if patch_refuse else 'ÉCHEC'}"
            )
    ...
    if not (is_acceptable(evaluations) and patch_refuse and boucle_ok):
