...
- **`agent_us2/`** : agent US2 (question libre), increment A1 (agent nu) terminé — `config.py`/`config.yml`, `tools.py` (outil `rechercher_regles`), `loop.py` (boucle ReAct tool-calling natif), `schemas.py`/`api.py`/`main.py` (API FastAPI, `POST /questions`, port 8882). Cette API est le premier endpoint de la future `api_business` partagée US1/US2 (routes `/questions/...`, puis `/audits/...` avec US1 — `conception/3_autre_us/en_commun.md`). Détail, schéma et contrat : `conception/3_autre_us/us2_question_libre/increments/A_agent_nu/`. Nommé `agent_us2` (pas `agent`) pour ne pas se confondre avec un futur `agent_us1`.
