...
- [x] **Valider pour de vrai l'export OTLP vers Langfuse Cloud (A2)** —
  soulevé le 2026-09-20 — `A` — clos 2026-09-20 : la validation réelle
  avait déjà été faite (`OTEL_EXPORTER=otlp`, `scripts/agent_cli.py`, trace
  `3bbed7a031660bc208006959659e7354` arrivée dans Langfuse Cloud avec le
  span racine `repondre` et ses enfants `appel_llm`/`appel_outil`), tracée
  dans `CHANGELOG.md` mais pas répercutée ici. Revérifiée le même jour par
  lecture de l'API v2 (`GET /api/public/v2/observations`) : 8 observations
  présentes, structure conforme. L'URL corrigée dans
  `_langfuse_otlp_config()` (`…/api/public/otel/v1/traces`) est donc
  confirmée par un vrai serveur. Les 6 sous-tâches Kanboard de la carte #24
  sont terminées. L'API legacy `/api/public/traces` est indisponible pour
  les organisations créées après le 16 septembre 2026 : utiliser la v2.
