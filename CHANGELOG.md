...
- **API données implémentée** (spec `docs/superpowers/specs/2026-07-26-api-fastapi-regles-design.md`, plan `docs/superpowers/plans/2026-07-26-api-regles-implementation.md`), 11 tâches TDD (test rouge → implémentation → test vert → commit par tâche)
  ...
  - **56 tests, tous verts** (`uv run pytest tests/unit/api_regles tests/unit/test_db.py tests/integration/api_regles`) : 7 configuration, 21 schémas, 4 authentification, 2 accès base, 22 intégration. Les tests d'intégration injectent leur session par `app.dependency_overrides[get_session]` : l'API sous test ne peut alors **pas** ouvrir de connexion vers `POSTGRES_DB`, garantie structurelle issue de l'incident du 2026-07-25. `ruff check` propre sur tout le périmètre
